...
{
    public class DynamicsOnlineClient
    {
        ...
        public DynamicsOnlineClient(bool isDebug)
        {
            var dbContext = new RCGAMZN_DEV01Entities();
            var dynamicsSetting = dbContext.DynamicsOnlineCMSConfigurations.FirstOrDefault();


            if (isDebug)
            {
                svc = new CrmServiceClient($"Url = {dynamicsSetting.BaseUrl_Dev}; AuthType = Office365; UserName = {dynamicsSetting.Username_Dev}; Password = {dynamicsSetting.Password_Dev}; RequireNewInstance = True");
            }
            else
            {
                // DynamicsOnlineCMSConfigurations needs to added in RCGAMZN_PROD01Entities
                svc = new CrmServiceClient($"Url = {dynamicsSetting.BaseUrl_Prod}; AuthType = Office365; UserName = {dynamicsSetting.Username_Prod}; Password = {dynamicsSetting.Password_Prod}; RequireNewInstance = True");
            }
        }

        public Guid AddOrUpdateCustomer(InCustomerProfileCM inCustomer)
        {
            ...
            account["new_clienttype"] = new OptionSetValue(CompanyTypeToDynamicsOptionSet(inCustomer.CompanyType.ToLower().Trim()));
            if (!updateAccount)
            ...
            // Contact Fields
            primaryContact["arb_3eentityid"] = ValidateStringLength(inCustomer.EntityID?.ToString(), 100);
