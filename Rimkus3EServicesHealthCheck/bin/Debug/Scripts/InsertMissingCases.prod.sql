...
INSERT INTO [TE_3E_RCG_SYNC].[dbo].TrackerSafeEMSImport
					...
					0 [Exported],
					o.Description,
					(select DisplayName from [RCG3ESQL31].[TE_3E_PROD].[dbo].[Timekeeper] where TkprIndex =  md.[BillTkpr]) as [Engineer1]
					from 
						[RCG3ESQL31].[TE_3E_PROD].[dbo].[Matter] m
						inner join [RCG3ESQL31].[TE_3E_PROD].[dbo].Client c on c.ClientIndex = m.Client
						inner join [RCG3ESQL31].[TE_3E_PROD].[dbo].[MattDate] md on md.MatterLkUp = m.MattIndex
						INNER JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].MattPayor mp ON mp.Matter = m.MattIndex
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].MattPayorDet mpd ON mpd.MattPayor=mp.MattPayorID AND mpd.CftRelationship_CCC in ('01', '03')
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].Payor p ON p.PayorIndex = mpd.Payor
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].Site locSite ON locSite.SiteIndex = mpd.Site
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].Address locAddress ON locAddress.AddrIndex = locSite.Address
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].Office o on o.Code = md.Office
						LEFT JOIN [RCG3ESQL31].[TE_3E_PROD].[dbo].Site oSite ON oSite.SiteIndex = o.Site
						LEFT join [RCG3ESQL31].[TE_3E_PROD].[dbo].Address adr on adr.AddrIndex = oSite.Address
